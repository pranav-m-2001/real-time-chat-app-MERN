...
function ChatContainer(): React.ReactNode{
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {
                    messages.map((message : any,index)=>(
                        <div key={index} className={`chat ${message.senderId === authUser._id ? 'chat-end' : 'chat-start'}`} ref={messageEndref}>
                            ...
                            <div className="chat-header mb-1">
                                <time className="text-xs opacity-50 mt-1">{formatDate(message.createdAt)}</time>
                            </div>
